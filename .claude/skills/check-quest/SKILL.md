...
- kill 的 `targets`（遇抵 uid 清單）為空、或某 uid 在任何地圖都找不到對應遇抵;地圖遇抵 `id` 缺失或全域重複;collect 的 `item`、reward 的 `items` 不在 `ItemCatalog`。
...
- **兩個任務「混在一起」** → 先確認不是共用 id(本驗證器會抓 id 指向錯誤)。**kill 已是逐遇抵 uid（不跨怪）**:殺別處同型怪不會算進別的任務,所以 kill 不會互擾。仍可能互擾的是 **collect**(背包持有數,共用):為任務 A 撿的同一道具會被任務 B 的 collect 追認 → 不同任務的 collect 用不同道具,或接受「做了就算」。

## 任務系統重點(寫/改任務內容時)

- 目標型別:`kill`(指定遇抵 uid 清單 `targets`,全部擊敗才算;**逐實例、不跨怪**——遇抵 uid 由 `tools/assign_encounter_uuids.gd` 自動生成)、`collect`(背包持有數,絕對追認)、`reach`(踏入指定 `map`+`pos`,事件式精確)、`talk`(對話 `advance_quest` 推進)。
