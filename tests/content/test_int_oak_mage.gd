...
func _load() -> MapData:
	return MapImporter.parse(FileAccess.get_file_as_string("res://content/maps/int_oak_mage.json"))
...
func test_vendor_at_counter():
	...
	assert_eq(m.get_vendor(Vector2i(7, 6))["id"], "oak_mage")

func test_exit_links_back_to_town():
	var m := _load()
	assert_eq(m.get_link(Vector2i(7, 11)), {"map": "town_oak", "entry": "oak_mage_out"})
