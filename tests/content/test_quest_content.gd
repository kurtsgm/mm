...
func test_goblin_menace_loads():
	...
	var wild_uid := ne.get_encounter_uid(Vector2i(5, 5))
	assert_eq(d.stage(0)["targets"], [wild_uid])
	assert_ne(wild_uid, oak.get_encounter_uid(Vector2i(9, 5)))
	assert_ne(wild_uid, "")
...
func test_town_oak_has_questgiver():
	...
	assert_not_null(map)
	# 守衛隊長（哥布林任務發包人）站在城門邊
	assert_true(map.has_quest_giver(Vector2i(6, 10)))
	assert_eq(map.get_quest_giver(Vector2i(6, 10))["dialogue"], "qg_oak_guard")
...
func test_wild_ne_has_goblin_and_chest():
	...
	assert_not_null(map)
	assert_eq(map.get_encounter(Vector2i(5, 5)), "g")
	assert_true(map.has_object(Vector2i(9, 5)))
	assert_eq(map.get_object(Vector2i(9, 5))["items"], ["lucky_charm"])
...
func test_giver_A_in_wild_nw_and_turnin_B_in_wild_ne():
	...
	assert_not_null(nw)
	assert_true(nw.has_quest_giver(Vector2i(5, 7)))
	assert_eq(nw.get_quest_giver(Vector2i(5, 7))["dialogue"], "qg_nw_messenger")
	var ne := MapImporter.parse(FileAccess.get_file_as_string("res://content/maps/wild_ne.json"))
	assert_not_null(ne)
	assert_true(ne.has_quest_giver(Vector2i(5, 9)))
	assert_eq(ne.get_quest_giver(Vector2i(5, 9))["dialogue"], "qg_ne_scout")
