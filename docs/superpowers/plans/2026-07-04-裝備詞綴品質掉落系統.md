...
**Interfaces:**
- Produces：`AffixCatalog.PREFIX/SUFFIX`、`AffixCatalog.entry(id)->Dictionary`、`AffixCatalog.eligible(category, ilvl, kind)->Array`（回 id 陣列）、`AffixCatalog.roll_mods(id, rng)->Dictionary`（stat_int→值）、`AffixCatalog.name_of(id)->String`。
...
**Interfaces:**
- Produces：`UniqueCatalog.entry(id)->Dictionary`、`UniqueCatalog.eligible(ilvl)->Array`。
