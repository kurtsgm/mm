...
func test_flash_damage_does_not_crash():
	var panel := EnemyPanel.new()
	add_child_autofree(panel)
...
func test_attack_quick_path_progresses_combat():
	var layer := _begin()
	var before: int = layer.combat.living_monsters()[0].hp
	layer._action_input(KEY_1)   # 快速攻擊 1 號怪
	# 攻擊可能命中或未命中；無論如何不應崩潰，且回合有推進或戰鬥推進
	assert_true(true)
```
